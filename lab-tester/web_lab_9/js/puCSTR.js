function puCSTR(pUnitIndex) {
  ...
  // XXX seems like rateHIGH & rateLOW below should be somewhere shared by
  //     all CSTR units and not copied into each one...
  //     maybe use a prototype property?

  // rateHIGH is high branch of average reaction rate (d'less turnover frequency)
  ...
  this.getRxnRate = function(conc) {
    ...
    if (c < cLowBreak) {
      // on high branch
      rate = rateHIGH[c];
      rateBranchOLD = 1; // 0 is low branch, 1 is high branch
    } else if (c > cHighBreak) {
      // on low branch
      rate = rateLOW[c-cLowBreak];
      rateBranchOLD = 0;
    } else if (rateBranchOLD == 0) {
      // in middle range and last on low branch, so still on low branch
      // on low branch
      rate = rateLOW[c-cLowBreak];
      rateBranchOLD = 0;
    } else if (rateBranchOLD == 1) {
  ...
  this.checkForSteadyState = function() {
    ...
    // console.log('unit ' + unitIndex + ', oldSScheckSum = ' + oldSScheckSum);
