...
const expImpCookie = {
    ...
    reportCookie: function() {
        console.log('enter reportCookie')
        const savedData = this.getCookie('flowsheetData');
        if (savedData) {
            // Parse JSON string back to object
            const flowsheetData = JSON.parse(decodeURIComponent(savedData));
            if (flowsheetData) {
                console.log('flowsheet data =', flowsheetData);
            } else {
                console.log('  oops! flowsheetData is null or undefined');
            }
        } else {
            console.log('oops! no cookie savedData!');
        }
    },

    importFlowsheet: function() {
        console.log('enter importFlowsheet');

        // if there are units already on the flowsheet
        // then don't want to overwrite copies
        console.log(`main.unitList.length = ${main.unitList.length}`);
        if (main.unitList.length > 0) {
            console.log(`  oops! flowsheet already has units, so exit `);
            return;
        }
        ...
        for (let n = 0; n < nmax; n++) {
            ...
            this.placeUnitsOnImport(unitID, unitObject, thisUnitCount, x, y, params);
        ...
        for (let j = 0; j < jmax; j++) {
            ...
            this.drawPipeOnImport(portINid, portOUTid);
        }; // END OF for loop to draw pipes 

        console.log('exit importFlowsheet');
    }, // END OF FUNCTION importFlowsheet

    placeUnitsOnImport: function(unitID, unitObject, unitCount, x, y, params) {
    ...
    drawPipeOnImport: function(portINid, portOUTid) {
...
} // end of object literal expImpCookie
