function puWaterTank(pUnitIndex) {
  ...
  // XXX EXPERIMENT XXX >> SEE const replacing let below
  ...
  // XXX EXPERIMENT XXX
  //
  // change this.level here to let level AND
  // change this.level everywhere else in this file to level
  let level = 0; // water level in this tank used by controller process unit
  this.getLevel = function() {return level}
  // SEE in water level controller change FROM processUnits[1].level TO
  //   processUnits[1].getLevel()
  ...
  this.updateState = function() {
    ...
    let exprValue = (level +
      unitTimeStep / Ax * (flowRate - newCoef * Math.pow(level,0.5)));
